...
namespace hashing
{
	...
	//	hashvalue
	//----------------------------------------------------------------------------
	bool SymbolTable::HasSymbolFor(const long long hashValue)
	{
	...
	//----------------------------------------------------------------------------
	const Symbol& SymbolTable::GetSymbolFor(const long long hashValue)
	{
	...
	{
		long long		hashValue = compute_hash(str);
	...
	{
		std::map<long long, Symbol const*>::const_iterator it;
		for (it = m_symbolMap.begin(); it != m_symbolMap.end(); ++it)
		{
			const Symbol& symbol = (*it->second);
			symbol.Print();
		}
		cout << "Hash collisions: " << m_Collisions;
	}
